...
const config = {
  ...
  reportPath: process.env.REPORT_PATH ? process.env.REPORT_PATH : './results',
  ...
  screenshotPath: process.env.SCREENSHOT_PATH ? process.env.SCREENSHOT_PATH : './results/screenshots'
}
...
After(async function (scenario) {
  ...
  if (scenario.result.status === Status.FAILED) {
    await this.page.screenshot({
      path: `${config.screenshotPath}/${screenShotName} (failed).png`
    })
  } else {
    await this.page.screenshot({
